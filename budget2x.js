 ...
 * @param {string} mccId The Customer ID of the MCC account to select.
 */
function applyBudgetsFromTab(spreadsheet, tabName, startReference, mccId) {
    ...
    try {
        // Attempt to select the child MCC account specified in the KeySheet.
        const accountIterator = MccApp.accounts().withIds([mccId]).get();
        if (accountIterator.hasNext()) {
            const childAccount = accountIterator.next();
            childAccount.selectAsCurrentAccount();
            Logger.log('  -> Successfully switched to child MCC account: ' +
                childAccount.getCustomerId());
        } else {
            Logger.log('  -> ERROR: MCC account with ID "' + mccId
                + '" was not found or is not accessible ' +
                'from this parent account. Skipping all campaign ' +
                'updates for tab "' + tabName + '".');
            return; // Exit this function if MCC is not found or accessible.
        }

        // Iterate over each campaign/budget/status triplet within the MCC.
        for (let i = 0; i < budgetValues.length; i++) {
            ...
            // selected MCC; setCampaignBudget now operates on the context
            // set by selectAsCurrentAccount().
            setCampaignBudget(campaignId, parseFloat(newBudget));
        }

    } catch (e) {
        // Catch any errors that occur during MCC selection or campaign
        // processing for this tab.
        Logger.log('  -> An error occurred while processing tab "' + tabName +
            '" for MCC "' + mccId + '": ' + e);
    } finally {
        // ALWAYS revert to the original parent account after processing a
        // child MCC's data. This is critical to ensure subsequent operations
        // or tabs are processed from the correct context.
        originalAccount.selectAsCurrentAccount();
        Logger.log('  -> Reverted to original parent account: ' +
            originalAccount.getCustomerId());
    }
}
 ...
 * This function now operates on the currently selected MCC account,
...
function setCampaignBudget(id, newBudget) {
    ...
    // set by applyBudgetsFromTab via MccApp.selectAsCurrentAccount().
    const campaignIterator = AdsApp.campaigns().withIds([id]).get();
    ...
    if (campaignIterator.totalNumEntities() === 0) {
        ...
        Logger.log('    -> ERROR: Campaign with ID "' + id +
            ...
            'This might indicate an incorrect Campaign ID or that ' +
            'it does not belong to this MCC.');
        return;
    }
